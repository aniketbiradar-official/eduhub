...
import {
  ...
  query, where, orderBy, serverTimestamp, updateDoc, arrayUnion, arrayRemove, getDoc
...
function Breadcrumb({ items, onNavigate }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'6px', flexWrap:'wrap', marginBottom:'24px', animation:'fadeUp 0.3s ease both' }}>
      {items.map((item, i) => (
...
// ── Course Card ─────────────────────────────────────────────────────────────
function CourseCard({ course, onClick, canEdit, onDelete, i }) {
  return (
    <div
      style={{
      ...
      onMouseEnter={e => {
        ...
        e.currentTarget.style.background = course.color + '10'
      }}
      onMouseLeave={e => {
    ...
    >
      {/* Top glow line */}
      <div style={{ position:'absolute', top:0, left:0, right:0, height:'2px', background:`linear-gradient(90deg, transparent, ${course.color}, transparent)`, opacity:0.6 }}/>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'12px' }}>
        <div>
          ...
          <div style={{ fontFamily:"'Syne',sans-serif", fontSize:'16px', fontWeight:'700', color:'white', marginBottom:'4px' }}>
            {course.name}
          </div>
          <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.35)' }}>
            {course.semesterCount || 0} semester{(course.semesterCount||0) !== 1 ? 's' : ''} · tap to explore
          </div>
        </div>
        {canEdit && (
          <button
            onClick={e => { e.stopPropagation(); onDelete(course) }}
            style={{ background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.15)',color:'rgba(239,68,68,0.5)',borderRadius:'8px',padding:'6px 8px',cursor:'pointer',fontSize:'13px',transition:'all 0.15s',flexShrink:0 }}
...
// ── Grid Card (semester, subject, folder) ───────────────────────────────────
function GridCard({ icon, label, sublabel, color, onClick, canDelete, onDelete, i }) {
  return (
    <div
      style={{
        background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)',
      ...
      onClick={onClick}
      onMouseEnter={e => { e.currentTarget.style.borderColor=(color||'rgba(255,255,255,0.2)'); e.currentTarget.style.transform='translateY(-2px)' }}
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'10px' }}>
        ...
        {canDelete && (
          <button
            ...
            style={{ background:'none',border:'none',color:'rgba(239,68,68,0.4)',cursor:'pointer',fontSize:'14px',padding:'4px',borderRadius:'6px',transition:'all 0.15s' }}
            onMouseEnter={e => e.target.style.color='#f87171'}
            onMouseLeave={e => e.target.style.color='rgba(239,68,68,0.4)'}
          >✕</button>
        )}
      </div>
      <div style={{ fontWeight:'500', color:'white', fontSize:'14px', marginBottom:'3px' }}>{label}</div>
      {sublabel && <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.3)' }}>{sublabel}</div>}
    </div>
  )
}
...
function FileRow({ file, canDelete, onDelete, i }) {
  ...
  return (
    <div style={{
      ...
      gap:'14px', animation:'fadeUp 0.3s ease both', animationDelay:`${i*0.05}s`,
    ...
    >
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontWeight:'500',color:'white',fontSize:'13px',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>
          {file.name}
        </div>
...
function UploadModal({ onClose, onUpload, uploading, progress }) {
  ...
  return (
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',alignItems:'center',justifyContent:'center',z:200,padding:'20px',backdropFilter:'blur(4px)',zIndex:200 }}
      ...
      <div style={{ background:'#0f1119',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'20px',padding:'32px',width:'100%',maxWidth:'460px' }}>
        ...
        >
          ...
          <div style={{color:'rgba(255,255,255,0.5)',fontSize:'13px'}}>
            Drop files here or <span style={{color:'#818cf8'}}>click to browse</span>
          </div>
          <div style={{color:'rgba(255,255,255,0.2)',fontSize:'11px',marginTop:'6px'}}>
            PDF, DOC, DOCX, PPT, PPTX, JPG, PNG · Max 5 files · 25MB each
          </div>
        </div>
          ...
          <div style={{marginBottom:'16px'}}>
            {files.map((f,i) => (
              <div key={i} style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 12px',background:'rgba(255,255,255,0.04)',borderRadius:'8px',marginBottom:'6px'}}>
                ...
                <button onClick={()=>removeFile(i)} style={{background:'none',border:'none',color:'rgba(239,68,68,0.5)',cursor:'pointer',fontSize:'14px',padding:'2px'}}>✕</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{display:'flex',gap:'10px'}}>
          <button onClick={onClose} style={{flex:1,padding:'11px',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'12px',background:'none',color:'rgba(255,255,255,0.5)',cursor:'pointer',fontSize:'14px',fontFamily:"'DM Sans',sans-serif"}}>
            Cancel
          </button>
          <button onClick={()=>onUpload(files)} disabled={uploading||files.length===0} style={{
            flex:2,padding:'11px',border:'none',borderRadius:'12px',
            background:uploading||files.length===0?'rgba(99,102,241,0.3)':'linear-gradient(135deg,#6366f1,#8b5cf6)',
            color:'white',cursor:uploading||files.length===0?'not-allowed':'pointer',
...
// ── Main FileSystem Page ─────────────────────────────────────────────────────
export default function FileSystem({ user, role }) {
  // Navigation state
  const [view, setView]           = useState('courses')   // courses | semesters | subjects | folder | files
  ...
  const [subjectCourses, setSubjectCourses] = useState([]) // for multi-course subject
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const unsub = onSnapshot(
      query(collection(db, 'semesters'), where('courseId', '==', selectedCourse.id), orderBy('number', 'asc')),
      snap => { setSemesters(snap.docs.map(d => ({ id: d.id, ...d.data() }))); setLoading(false) }
    )
    return () => unsub()
  }, [selectedCourse])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const unsub = onSnapshot(
      query(collection(db, 'subjects'), where('semesterIds', 'array-contains', selectedSem.id), orderBy('name', 'asc')),
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      query(
        collection(db, 'fs_files'),
        where('subjectId', '==', selectedSubject.id),
        where('folder', '==', selectedFolder.id),
        orderBy('createdAt', 'desc')
      ),
      snap => { setFiles(snap.docs.map(d => ({ id: d.id, ...d.data() }))); setLoading(false) }
    )
    return () => unsub()
  ...
  const breadcrumb = [
    ...
    ...(selectedSem    ? [`Semester ${selectedSem.number}`] : []),
  ...
  const handleBreadcrumb = (i) => {
    if (i === 0) goTo('courses')
    if (i === 1) goTo('semesters', selectedCourse)
    if (i === 2) goTo('subjects', selectedSem)
    if (i === 3) goTo('folders', selectedSubject)
  }
  ...
  const deleteCourse = async (course) => {
    if (!confirm(`Delete course "${course.name}"? This won't delete uploaded files.`)) return
  ...
  const addSemester = async () => {
    if (!addSemNum) return
    const existing = semesters.find(s => s.number === parseInt(addSemNum))
    if (existing) return showToast('This semester already exists.')
    await addDoc(collection(db, 'semesters'), {
      number: parseInt(addSemNum), courseId: selectedCourse.id,
  ...
  const deleteSemester = async (sem) => {
    ...
    await deleteDoc(doc(db, 'semesters', sem.id))
    showToast('Semester deleted.')
  }

  // ── CRUD: Subject ─────────────────────────────────────────────────────────
  const addSubject = async () => {
    if (!addName.trim() || subjectCourses.length === 0) return showToast('Enter a name and pick at least one semester.')
    await addDoc(collection(db, 'subjects'), {
      name: addName.trim(),
      semesterIds: subjectCourses,  // array of semesterIds this subject belongs to
      createdAt: serverTimestamp(), createdBy: user.uid,
    })
    setAddName(''); setSubjectCourses([]); setShowAddSubject(false)
  ...
  const handleUpload = async (fileList) => {
    ...
    for (const file of fileList) {
      if (file.size > 26214400) { showToast(`${file.name} is too large (max 25MB)`); continue }
        ...
        task.on('state_changed',
          ...
          async () => {
            ...
            await addDoc(collection(db, 'fs_files'), {
              ...
              uploadedBy: user.displayName, uploadedByUid: user.uid,
  ...
  const handleDeleteFile = async (file) => {
    const canDel = can(role, 'delete_any_notes') || file.uploadedByUid === user.uid
    if (!canDel) return showToast("You can only delete your own uploads.")
    if (!confirm(`Delete "${file.name}"?`)) return
    try {
      const storageRef = ref(storage, file.storagePath)
      await deleteObject(storageRef).catch(() => {})
      await deleteDoc(doc(db, 'fs_files', file.id))
  ...
  const renderSkeleton = (n=3) => Array.from({length:n}).map((_,i) => (
    <div key={i} style={{background:'rgba(255,255,255,0.04)',borderRadius:'14px',height:'90px',animation:'pulse 1.5s ease-in-out infinite',animationDelay:`${i*0.1}s`}}/>
  ))
  ...
  return (
    <>
      <style>{`
        ...
        .fs-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:14px}
        ...
        .primary-btn{background:linear-gradient(135deg,#6366f1,#8b5cf6);color:white;border:none;border-radius:10px;padding:9px 18px;font-size:13px;font-weight:500;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all 0.15s}
        .primary-btn:hover{opacity:0.9;transform:translateY(-1px)}
        .primary-btn:disabled{opacity:0.4;cursor:not-allowed;transform:none}
        .outline-btn{background:transparent;color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.12);border-radius:10px;padding:9px 18px;font-size:13px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all 0.15s}
        .outline-btn:hover{background:rgba(255,255,255,0.06);color:white}
        .section-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:10px;animation:fadeUp 0.3s ease both}
        .section-title{font-family:'Syne',sans-serif;font-size:'22px';font-weight:700;color:white}
        ...
        .sem-chips{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px}
        .sem-chip{padding:5px 12px;border-radius:8px;font-size:12px;cursor:pointer;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.4);transition:all 0.15s;font-family:'DM Sans',sans-serif}
        .sem-chip.sel{background:rgba(99,102,241,0.2);color:#a5b4fc;border-color:rgba(99,102,241,0.35)}
      `}</style>
      ...
      <div style={{marginBottom:'8px',animation:'fadeUp 0.3s ease both'}}>
        ...
        <p style={{color:'rgba(255,255,255,0.35)',fontSize:'14px'}}>
          Browse Course → Semester → Subject → Folder
        </p>
      </div>
      ...
      {view === 'courses' && (
        <>
          <div className="section-header">
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:'20px',fontWeight:'700',color:'white'}}>
              All Courses <span style={{fontSize:'14px',color:'rgba(255,255,255,0.3)',fontWeight:'400',marginLeft:'8px'}}>{courses.length} courses</span>
            </div>
            {canEdit && (
              <button className="primary-btn" onClick={()=>setShowAddCourse(s=>!s)}>
          ...
          {showAddCourse && (
            <div className="add-form">
              <div style={{fontSize:'13px',color:'rgba(255,255,255,0.5)',marginBottom:'10px'}}>New course</div>
              <input className="fi" placeholder="Course name (e.g. BSc CS, BCA, MCA...)" value={addName} onChange={e=>setAddName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addCourse()}/>
              <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)',marginBottom:'6px'}}>Pick a color</div>
              <div className="color-row">
                {COLORS.map(c => (
                  <div key={c} className="cdot" style={{background:c,borderColor:addColor===c?'white':'transparent'}} onClick={()=>setAddColor(c)}/>
                ))}
              </div>
              <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
                <button className="primary-btn" onClick={addCourse} disabled={!addName.trim()}>Create Course</button>
                <button className="outline-btn" onClick={()=>setShowAddCourse(false)}>Cancel</button>
              </div>
            </div>
          )}

          <div className="fs-grid">
            {courses.length === 0 ? renderEmpty('No courses yet', canEdit ? 'Add your first course above!' : 'Your CR will add courses here.') :
             courses.map((c,i) => (
               <CourseCard key={c.id} course={c} i={i}
      ...
      {view === 'semesters' && (
        <>
          <div className="section-header">
            <div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:'20px',fontWeight:'700',color:'white'}}>Semesters</div>
              <div style={{fontSize:'13px',color:'rgba(255,255,255,0.35)',marginTop:'2px'}}>{selectedCourse?.name}</div>
            </div>
            {canEdit && (
              <button className="primary-btn" onClick={()=>setShowAddSem(s=>!s)}>
                {showAddSem ? '✕ Cancel' : '+ Add Semester'}
              </button>
            )}
          </div>
          ...
          {showAddSem && (
            <div className="add-form">
              <div style={{fontSize:'13px',color:'rgba(255,255,255,0.5)',marginBottom:'10px'}}>New semester</div>
              <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)',marginBottom:'8px'}}>Select semester number</div>
              <div className="sem-chips">
                {[1,2,3,4,5,6,7,8].map(n => (
                  <button key={n} className={`sem-chip ${addSemNum===String(n)?'sel':''}`} onClick={()=>setAddSemNum(String(n))}>
                    Sem {n}
                  </button>
                ))}
              </div>
              <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
                <button className="primary-btn" onClick={addSemester}>Create Semester</button>
                <button className="outline-btn" onClick={()=>setShowAddSem(false)}>Cancel</button>
              </div>
            </div>
          )}
          ...
          <div className="fs-grid">
            {loading ? renderSkeleton(4) :
             semesters.length === 0 ? renderEmpty('No semesters yet', canEdit ? 'Add a semester above!' : 'Your CR will add semesters here.') :
             semesters.map((s,i) => (
               <GridCard key={s.id} icon="📅" label={`Semester ${s.number}`} sublabel="Tap to see subjects" i={i}
                 color="#6366f1"
                 onClick={()=>goTo('subjects', s)}
                 canDelete={canEdit}
                 onDelete={()=>deleteSemester(s)}
               />
             ))
            }
          </div>
      ...
      {view === 'subjects' && (
        <>
          <div className="section-header">
            <div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:'20px',fontWeight:'700',color:'white'}}>Subjects</div>
              <div style={{fontSize:'13px',color:'rgba(255,255,255,0.35)',marginTop:'2px'}}>
                {selectedCourse?.name} · Semester {selectedSem?.number}
              </div>
            </div>
            {canEdit && (
              <button className="primary-btn" onClick={()=>setShowAddSubject(s=>!s)}>
          ...
          {showAddSubject && (
            <div className="add-form">
              <div style={{fontSize:'13px',color:'rgba(255,255,255,0.5)',marginBottom:'10px'}}>New subject</div>
              <input className="fi" placeholder="Subject name (e.g. Data Structures, DBMS...)" value={addName} onChange={e=>setAddName(e.target.value)}/>
              <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)',marginBottom:'8px'}}>
                Also add to other semesters? (optional — for shared subjects)
              </div>
              <div className="sem-chips">
                {semesters.map(s => (
                  <button key={s.id}
                    className={`sem-chip ${subjectCourses.includes(s.id)||s.id===selectedSem.id?'sel':''}`}
                    onClick={()=>{
                      if (s.id===selectedSem.id) return
                      setSubjectCourses(prev => prev.includes(s.id) ? prev.filter(x=>x!==s.id) : [...prev, s.id])
                    }}
                    disabled={s.id===selectedSem.id}
                  >
                    Sem {s.number} {s.id===selectedSem.id?'(current)':''}
                  </button>
                ))}
              </div>
              <div style={{display:'flex',gap:'8px',marginTop:'8px'}}>
                <button className="primary-btn" onClick={addSubject} disabled={!addName.trim()}>Create Subject</button>
                <button className="outline-btn" onClick={()=>{setShowAddSubject(false);setAddName('');setSubjectCourses([])}}>Cancel</button>
              </div>
            </div>
          )}
          ...
          <div className="fs-grid">
            {loading ? renderSkeleton(4) :
             subjects.length === 0 ? renderEmpty('No subjects yet', canEdit ? 'Add a subject above!' : 'Your CR will add subjects here.') :
             subjects.map((s,i) => (
               <GridCard key={s.id} icon="📚" label={s.name}
                 sublabel={s.semesterIds?.length > 1 ? `Shared in ${s.semesterIds.length} semesters` : 'Tap to see folders'}
                 color="#10b981" i={i}
        ...
        <>
          <div className="section-header">
            <div>
              ...
              <div style={{fontSize:'13px',color:'rgba(255,255,255,0.35)',marginTop:'2px'}}>
                Choose a folder to view or upload files
              </div>
            </div>
          </div>
      ...
      {view === 'files' && (
        <>
          <div className="section-header">
            <div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:'20px',fontWeight:'700',color:'white',display:'flex',alignItems:'center',gap:'10px'}}>
                <span>{selectedFolder?.icon}</span> {selectedFolder?.label}
              </div>
              <div style={{fontSize:'13px',color:'rgba(255,255,255,0.35)',marginTop:'2px'}}>
                {selectedSubject?.name} · {files.length} file{files.length!==1?'s':''}
              </div>
            </div>
            {canEdit && (
            ...
            {loading ? renderSkeleton(3) :
             files.length === 0 ? (
               <div style={{textAlign:'center',padding:'60px 20px',color:'rgba(255,255,255,0.2)'}}>
                 ...
                 <div style={{fontSize:'15px',fontWeight:'500',color:'rgba(255,255,255,0.3)',marginBottom:'6px'}}>
                   No files in {selectedFolder?.label} yet
                 </div>
                 <div style={{fontSize:'13px'}}>
                   {canEdit ? 'Click "Upload Files" to add the first one!' : 'Your CR will upload files here.'}
                 </div>
               </div>
             ) :
             files.map((f,i) => (
